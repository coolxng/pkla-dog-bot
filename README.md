...
| `PIPER_TTS_MODEL` | unset | Path to the local Piper `.onnx` voice model used for the default **Speak in call** and `!tts` voice. Required for the default voice to synthesize audio. |
| `PIPER_TTS_MANLY_MODEL` | unset | Optional path to a lower/deeper Piper `.onnx` voice model. When set, `/say` shows a **Manly Piper** voice option and `PIPER_TTS_VOICE=manly` makes it the default for `/say` and `!tts`. |
| `PIPER_TTS_BINARY` | `piper` | Piper executable to run for local text-to-speech. |
| `PIPER_TTS_VOICE` | `default` | Default voice selected on `/say`. Use `manly` after setting `PIPER_TTS_MANLY_MODEL`; unsupported values fall back to `default`. |
...
| `!tts <message>` | When enabled from `/say`, queues up to 500 characters to be read with the configured Piper default voice in the connected voice channel. Multiple `!tts` messages play in order without overlapping. |
...
Normal chat and optional automatic memory extraction use the Groq API associated with `GROQ_API_KEY`. Text-to-speech runs through the local Piper executable configured by `PIPER_TTS_BINARY`, `PIPER_TTS_MODEL`, and optional `PIPER_TTS_MANLY_MODEL`; the existing OpenAI web-search provider runs only when `ENABLE_OPENAI_WEB_SEARCH=true`. Browser listening does not call either AI provider.
